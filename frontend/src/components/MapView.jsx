...
export default function MapView({ selectedPickup, onPickupSelect }) {
  ...
  return (
    <main className={`rq-professional-map ${isDashboardMap ? "in-dashboard" : ""} ${dark ? "dark" : "light"}`}>
      <style>{`
        .rq-professional-map { --panel:#fff; --text:#17251c; --muted:#66776c; --line:rgba(19,74,39,.13); --soft:#f4f8f5; position:relative; height:calc(100dvh - 72px); margin-top:72px; overflow:hidden; background:#e8efe9; color:var(--text); font-family:'Manrope',system-ui,sans-serif; }
