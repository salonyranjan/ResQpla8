  ...
  return <>
    <header className={`main-navbar ${scrolled ? "is-scrolled" : ""}`}>
      <div className="main-navbar-inner">
        ...
        <nav className="desktop-navigation" aria-label="Main navigation">
          {links.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `main-nav-link ${isActive ? "active" : ""}`}>{item.label}</NavLink>)}
        </nav>
        <div className="main-navbar-actions">
          ...
          <div className="desktop-account-actions">
            {isAuthenticated ? <Link to="/dashboard" className="account-link"><span className="account-avatar">{initials}</span><span className="account-copy"><small>Signed in</small>{user?.name || "Dashboard"}</span></Link> : <><Link to="/login" className="login-link">Log in</Link><Link to="/register" className="primary-nav-action">Get started</Link></>}
          </div>
    ...
    <aside id="mobile-navigation" className={`mobile-navigation ${open ? "open" : ""}`} aria-hidden={!open}>
      ...
      <nav className="mobile-navigation-links" aria-label="Mobile navigation">
        {links.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `mobile-nav-link ${isActive ? "active" : ""}`} tabIndex={open ? 0 : -1}>{item.label}<span>→</span></NavLink>)}
        {isAuthenticated && <NavLink to="/dashboard" className="mobile-nav-link" tabIndex={open ? 0 : -1}>Dashboard<span>→</span></NavLink>}
      </nav>
    ...
    <style>{`
      ...
      .main-nav-link{padding:8px 15px;border-radius:9px;color:${dark ? "#aab8ae" : "#536158"};font-size:14px;font-weight:600;text-decoration:none}.main-nav-link:hover{color:${dark ? "#fff" : "#132419"}}.main-nav-link.active{background:${dark ? "rgba(82,183,136,.15)" : "#e1ecdf"};color:${dark ? "#8dd5aa" : "#205f43"}}
      ...
      .login-link,.primary-nav-action{padding:9px 16px;border-radius:10px;font-size:14px;font-weight:700;text-decoration:none}.login-link{color:${dark ? "#dce6df" : "#26392c"}}.primary-nav-action{background:#2d6a4f;color:#fff}.account-link{gap:9px;padding:5px 10px 5px 5px;border-radius:12px;color:${dark ? "#f0f5f1" : "#17271c"};text-decoration:none}.account-avatar{display:grid;place-items:center;flex:0 0 34px;height:34px;border-radius:10px;background:#2d6a4f;color:#fff;font-size:12px;font-weight:800}.account-copy{max-width:140px;display:flex;flex-direction:column;font-size:13px;font-weight:700;line-height:1.25;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.account-copy small{font-size:10px;font-weight:500;color:${dark ? "#91a097" : "#6b776f"}}
